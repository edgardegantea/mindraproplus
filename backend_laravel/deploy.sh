...
echo "[1/9] Creating storage directories..."
...
echo "[3/9] Running post-install scripts..."
...
if [ ! -f .env ]; then
    ...
    echo "[4/9] .env already exists, merging new variables..."
...
echo "[8/9] Building production cache..."
...
# 9. Restart queue worker (picks up new code; no-op if not running)
echo "[9/9] Restarting queue worker..."
php artisan queue:restart || echo "  (queue:restart sent — worker will restart on next job)"
...
echo ""
echo "Si persiste algún 500, las últimas líneas del log son:"
echo "---"
tail -30 storage/logs/laravel.log 2>/dev/null || echo "(log vacío o inaccesible)"
