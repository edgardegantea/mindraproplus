...
<div style="background:#fff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;">
    <div style="padding:20px 24px;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;justify-content:space-between;">
        <div>
            <h3 style="font-size:1rem;font-weight:700;color:#0f172a;margin:0;">Todas las sesiones de la institución</h3>
            <p style="font-size:.8125rem;color:#94a3b8;margin:4px 0 0;">Historial completo de interacciones de tus usuarios con Mindra</p>
        </div>
        <form method="POST" action="{{ route('admin.sessions.export') }}">
            @csrf
            <button type="submit" style="padding:8px 16px;border:1px solid #e2e8f0;border-radius:8px;background:#fff;font-size:.75rem;font-weight:600;color:#334155;cursor:pointer;">Exportar CSV</button>
        </form>
    </div>
        ...
        <tbody>
            ...
            <tr style="border-bottom:1px solid #f8fafc;" onmouseover="this.style.background='#fafbff'" onmouseout="this.style.background='transparent'">
                ...
                <td style="padding:12px 16px;">
                    <a href="{{ route('admin.user', $record->user) }}" style="font-size:.8125rem;font-weight:600;color:#4f46e5;">{{ $record->user?->name ?? '—' }}</a>
                </td>
                <td style="padding:12px 16px;font-size:.8125rem;color:#334155;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $record->input_text ?: '(audio)' }}</td>
                <td style="padding:12px 16px;font-size:.8125rem;color:#64748b;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $record->generated_text ?: '—' }}</td>
                ...
                <td style="padding:12px 16px;text-align:center;">
                    <form method="POST" action="{{ route('admin.sessions.delete', $record) }}" onsubmit="return confirm('¿Eliminar esta sesión?');" style="display:inline;">
                        @csrf
            ...
            @empty
            <tr><td colspan="7" style="padding:48px;text-align:center;color:#94a3b8;">Sin sesiones registradas.</td></tr>
            @endforelse
        </tbody>
    </table>
